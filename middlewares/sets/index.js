...
exports.validatorRules = (isNew = true) => [
    ...
    body("cards.*.front", "Card 'front' is required").trim().exists({ checkFalsy: true }),
    body("cards.*.front", "Card 'front' cannot exceed character limit of 250.").isLength({
        max: 250,
    }),
    body("cards.*.back", "Card 'back' is required").trim().exists({ checkFalsy: true }),
    body("cards.*.back", "Card 'back' cannot exceed character limit of 250.").isLength({
        max: 250,
    }),
